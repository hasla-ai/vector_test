...
```bash
=== [2장-1강] 문제 3-1 출력 결과 ===
--- [1] 가산성 / 동차성 / 원점 보존 검증 표 ---
검증 항목              | T_linear (선형 변환)     | T_affine (아핀 변환)    
-----------------------------------------------------------------
1. 가산성 T(u+v)=T(u)+T(v) | True                 | False               
2. 동차성 T(c·u)=c·T(u) | True                 | False               
3. 원점 보존 T(0)=0    | True                 | False               
-----------------------------------------------------------------

=== 선형 변환 미성립 이유 및 딥러닝에서의 필요성 ===
...
![문제 3-1 선형 변환과 아핀 변환 비교 산점도](images\chapter_2_1_problem_3_1_linear_affine_transformation.png)

- 행렬 곱만 하는 변환은 가산성·동차성을 모두 만족하고 원점을 원점으로 보내므로 **선형 변환**입니다.
- 평행이동을 더한 affine 변환은 세 조건을 모두 어깁니다. 특히 `T(0) = b ≠ 0`이므로 **원점을 원점으로 보내지 않아** 선형 변환이 될 수 없습니다.
- 그럼에도 신경망이 `XW + b`를 쓰는 이유는, 편향 `b`가 결정 경계를 원점에 묶이지 않게 해 표현력을 높이기 때문입니다. 다만 선형 변환만 여러 층 쌓으면 결국 하나의 선형 변환과 같아지므로, 층 사이에 비선형 활성화 함수가 필요합니다.
