...
print("\n" + "="*50)
print("[필수 2] 문제 2-2: 고유분해와 SVD의 적용 범위 비교")
print("="*50)
...
try:
    ...
    print(f"직사각행렬 M 고유분해 오류: {e}")

# 2. SVD 성공 확인
print(f"직사각행렬 M SVD 분해 성공 여부: U={U.shape}, S={S.shape}, Vt={Vt.shape}")

# 3. 대각화 불가능한 B 행렬 SVD
B = np.array([[1.0, 1.0], [0.0, 1.0]])
_, S_B, _ = np.linalg.svd(B)
print(f"대각화 불가 행렬 B의 특이값: {S_B}")

# 4. 비교 표
print("\n[고유분해 vs SVD 비교 표]")
comp_data = {
    '구분': ['고유분해 (Eigendecomposition)', 'SVD (특이값 분해)'],
    '적용 대상': ['정사각행렬 (대각화 가능한 경우만)', '모든 직사각/정사각 행렬'],
    '결과 벡터 직교성': ['일반적으로 직교하지 않음 (대칭행렬 제외)', '좌/우 특이벡터 모두 항상 정규직교'],
    '수치 안정성': ['조건수가 나쁘면 수치적으로 불안정', '직교행렬을 사용하므로 매우 안정적']
}
print(pd.DataFrame(comp_data).to_string(index=False))

# 5. SVD 선택 기준 작성
print("\n[선택 기준] 입력 행렬이 직사각행렬이거나, 행렬의 차원 축소 및 저랭크 근사를 직교 기저 기반으로 안정하게 수행해야 할 때 SVD를 선택해야 합니다.")
