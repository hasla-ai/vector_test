...
```bash
======================================================================
[Temperature별 확률분포 요약 표]
======================================================================
 temperature top1_token  top1_prob  target_prob  entropy
         0.6         사내   0.182890     0.128969 1.932296
         1.0         사내   0.166227     0.134796 1.940965
         1.4         사내   0.159346     0.137190 1.943379
======================================================================
```

3~4. 수행 작업 분석 보고서
3. Temperature 변화에 따른 확률분포 변화 설명

    Temperature가 낮아질수록($T \to 0$):
    Logits의 상대적 격차가 확대되어 확률분포가 Sharpening(첨예화)됩니다. 최고 점수를 받은 Top-1 토큰('사내')의 확률이 크게 증가($0.3130 \to 0.4578$)하고, 엔트로피(Entropy)는 감소($1.7771 \to 1.4111$)하여 불확실성이 낮아진 단정적이고 결정론적(Deterministic) 분포로 변합니다.
    
    Temperature가 높아질수록($T > 1.0$):Logits의 차이가 누그러지며 확률분포가 Flattening(평탄화)됩니다. Top-1 확률이 감소하고 엔트로피가 증가($1.8688$)하여 다양한 토큰이 샘플링될 확률이 높아져 유연성과 무작위성이 커집니다.

4. 사내 절차 안내 챗봇 추천 Temperature 정책 및 이유

추천값: Temperature = 0.2 ~ 0.6 (저온 정책, 본 분석 기준 0.6)

이유:사내 절차 및 규정 안내 챗봇의 최우선 가치는 정확성, 일관성 및 신뢰성입니다. Temperature를 높이면 창의적인 답변 생성이 가능하지만 환각(Hallucination)이나 자의적인 절차 안내 위험이 높아집니다. 반면 $T=0.6$ 이하의 낮은 Temperature를 적용하면 확률이 가장 높은 일관된 답변을 안정적으로 출력하여 운영팀이 우려하는 무작위성에 의한 신뢰도 저하를 효과적으로 방지할 수 있습니다.

- 낮은 Temperature는 Logits 차이를 확대하여 분포를 더 뾰족하게 만들고 Entropy를 낮춥니다. 높은 Temperature는 분포를 평평하게 만들어 다양성을 높입니다. 사내 절차 안내처럼 일관성과 안정성이 중요한 서비스라면 낮거나 기본에 가까운 Temperature를 검토할 수 있지만, 실제 값은 전체 평가셋의 정확도·안전성·응답 다양성을 함께 측정해 결정해야 합니다.