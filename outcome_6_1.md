...
# 심화 1. `d_k`가 커질 때 Scaling의 효과를 수치로 확인하기

## ▶ 문제 3-1: Score 분산 모니터링 실험

### 업무 상황

모델 구조를 바꾸면서 `d_k`가 4, 16, 64, 256으로 커졌습니다. 운영팀은 `QKᵀ` 점수의 표준편차가 차원에 따라 얼마나 커지는지, `√d_k` Scaling 후에는 안정되는지 확인하려고 합니다.

### 수행해야 할 작업

1. 각 `d_k`에서 표준정규분포로 Query와 Key 벡터 5,000쌍을 만드세요.
2. 각 쌍의 내적을 계산하세요.
3. Scaling 전후 표준편차를 기록하세요.
4. 막대그래프로 비교하세요.
5. 다음 질문에 답하세요.
    - `d_k`가 커질수록 Scaling 전 표준편차는 어떻게 변하나요?
    - Scaling 후 표준편차가 약 1 근처에 머무는 이유는 무엇인가요?

- 각 표본에서 내적은 `np.sum(q * k, axis=1)`로 계산할 수 있습니다.
- 난수 생성기는 `np.random.default_rng(seed)`를 사용하세요.
- 결과를 리스트에 딕셔너리 형태로 저장하면 그래프를 그리기 편합니다.

결과

```bash
============================================================
[차원(d_k)별 Score 표준편차 모니터링 결과]
============================================================
 d_k   raw_std  scaled_std
   4  2.008743    1.004371
  16  4.061580    1.015395
  64  7.881494    0.985187
 256 16.199444    1.012465
```

![images\chapter_6_1_problem_3_1_plot_monitor_attention_score_variance_stabilization_across_d_k_dimensions](images\chapter_6_1_problem_3_1_plot_monitor_attention_score_variance_stabilization_across_d_k_dimensions.png)

독립적인 성분의 곱을 d_k개 더하면 내적의 분산이 대략 d_k에 비례합니다. 따라서 표준편차는 √d_k에 비례해 커집니다. 이를 다시 √d_k로 나누면 점수의 표준편차가 차원과 무관하게 약 1 근처로 안정됩니다.
